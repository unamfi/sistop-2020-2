...
def FCFS(process_queue): #Algoritmo que es el FCFS (First Come First Serve)
    l = len(process_queue) #Almacenamos la longitud del arreglo de procesos
    t = [process_queue[i][2] for i in range(l)] #Tiempo total que dura el proceso

    '''Las variables representan el tiempo de respuesta, espera y la proporción de penalización (T, E, P respectivamente)
    Rango de 0 hasta el valor máximo 'l' que es la longitud del nuúmero de procesos'''
    ...
    print ('FCFS:   T = %.2f' % averageT,'\t', 'E = %.2f' % averageE,'\t', 'P = %.2f' % averageP) #Imprime la tabla y los valores promedio
...
#Intenté implementar SPN, pero no entendí bien como hacer el algoritmo. Dejo comentado lo hasta lo que hice
'''def SPN(process_queue):
    ...
    begin = [process_queue[i][2] for i in range(l)]
    End = [0 for i in range(l)]
...
rondas = random.randint(1,20)   #Obtenemos el número aleatorio entre 1 y 20 para el numero de rondas de cada proceso
