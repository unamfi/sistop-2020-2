...
class FileSystem:
    ...
    def valida_FS(self):
        ...
        for key in self.FS_info:
            ...
            if  not data in list(self.FS_info.keys()):
                raise Exception ('Verifique su archivo')
        return
    #Para obtener los dato del cluster
    def getDataSB(self):
        for key in self.SB:
            value = self.SB[key]
            self.file.seek(value[0])
    ...
    #Muestra el listado de todos los archivos
    def ls(self):
        self.Files = []
        self.getFilesInfo()
    ...
    def copyToFS(self,path):
        ...
        if len(nombre)>15:
            print('El nombre del archivo es muy grande')
            return
